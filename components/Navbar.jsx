...
function Navbar() {
  ...
  const navItems = [
    ...
    { label: "Construction Material & items", link: "/construction-material", dropdown: false },
  ...
  return (
    <div className="container mt-5">
      <div className="flex justify-between items-center">
        ...
        <div className="hidden md:flex menu-bar justify-between items-center">
          {navItems.map((navItem, idx) => (
            !navItem.dropdown ? (
              <Menubar className="text-sm font-medium border-none p-3 hover:text-primary" key={idx}>
                <Link href={navItem.link}>{navItem.label}</Link>
              </Menubar>
            ) : (
              <div className="flex items-center ml-4" key={idx}>

              <Menubar className="border-none hover:text-primary flex items-center" key={idx}>
                <MenubarMenu>
      ...
      <div className="pt-2 pb-4">
          ...
          {navItems.map((navItem, idx) => (
            !navItem.dropdown ? (
              <Menubar className="text-sm font-medium border-none hover:text-primary flex items-center" key={idx}>
                <Link href={navItem.link}>{navItem.label}</Link>
              </Menubar>
            ) : (
              <Menubar className="border-none hover:text-primary flex items-start" key={idx}>
                <MenubarMenu>
                  <MenubarTrigger>{navItem.label}</MenubarTrigger>
                  <MenubarContent>
                    {navItem.children.map((child, idx) => (
